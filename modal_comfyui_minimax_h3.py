...
image = (
    ...
    .run_commands(
        ...
        "python3 - <<'EOF'\nimport sys\np = '/ComfyUI/custom_nodes/Comfyui_Minimax_h3_latent_Upscaler/nodes/minimax_h3_latent_upscaler_2d.py'\ntry:\n    with open(p) as f:\n        src = f.read()\nexcept FileNotFoundError:\n    print(f'WARN: {p} not found — skipping upscaler patch', file=sys.stderr)\n    sys.exit(0)\nold = '        s = latent[\"samples\"].clone()\\n        orig_dtype = s.dtype'\nnew = '''        samples_in = latent[\"samples\"]\n        # DEBUG 2026-09-18 (LowUpscaled revival): print real type + attrs\n        # coming from H3 sampler (node 151). Revert after diagnosis.\n        print(f\"[UPSCALER-DEBUG] type={type(samples_in).__module__}.{type(samples_in).__name__}\",\n              f\"shape={getattr(samples_in, 'shape', None)}\",\n              f\"has_is_nested={hasattr(samples_in, 'is_nested')}\",\n              f\"has_to_padded_tensor={hasattr(samples_in, 'to_padded_tensor')}\",\n              f\"has_clone={hasattr(samples_in, 'clone')}\",\n              f\"has_to_dense={hasattr(samples_in, 'to_dense')}\",\n              f\"is_nested_val={getattr(samples_in, 'is_nested', 'N/A')}\",\n              flush=True)\n        # Patch 2026-09-18 (R.109f): per-component pad, return as list (was stack). R.109c: `to_padded_tensor(NT)` -> 'not NestedTensor'. R.109d: `to_padded_tensor(list)` -> 'not list'. R.109e: stack() failed because NT mixes 5D video + 4D audio constituents (audio has no T dim). ComfyUI Upscaler iterates constituents via _async_map_node_over_list — return a LIST of padded tensors, one per constituent, padded on T if present.\n        if hasattr(samples_in, 'is_nested') and samples_in.is_nested:\n            import torch as _torch\n            components = list(samples_in.unbind())  # list of plain Tensors\n            # ComfyUI Upscaler iterates constituents via _async_map_node_over_list.\n            # R.109e stacked them; failed because NT mixes 5D video [1,24,T,H,W] and 4D audio [1,57,2,150]\n            # constituents (audio latent has no T dim). Per-component pad the T dim if present,\n            # then return the LIST — ComfyUI iterates it.\n            max_t = max(c.shape[2] for c in components if c.ndim >= 3)\n            padded = []\n            for c in components:\n                if c.ndim >= 3 and c.shape[2] < max_t:\n                    pad_amt = max_t - c.shape[2]\n                    # F.pad takes pad pairs from the END of shape.\n                    # For ndim=5 [B,C,T,H,W]: pad=(0,0, 0,0, 0,pad_amt, 0,0, 0,0).\n                    # For ndim=4 [B,C,T,H]: pad=(0,0, 0,0, 0,pad_amt, 0,0).\n                    n_pad_pairs = c.ndim\n                    pad_arg = [0, 0] * n_pad_pairs\n                    # T is dim 2; pad pair for T is at indices (2*n_pad_pairs - 5, 2*n_pad_pairs - 4)\n                    # i.e. for ndim=5: pad_arg[5]=pad_amt; for ndim=4: pad_arg[3]=pad_amt.\n                    pad_arg[2*n_pad_pairs - 5] = pad_amt\n                    c = _torch.nn.functional.pad(c, tuple(pad_arg), value=0.0)\n                padded.append(c)\n            samples_in = padded  # R.109f — return list of padded plain Tensors (was stack; failed on mixed-dim NT)\n        s = samples_in.clone()\n        orig_dtype = s.dtype'''\nif old in src:\n    src = src.replace(old, new)\n    with open(p, 'w') as f:\n        f.write(src)\n    print('Patched: MinimaxH3LatentUpscalerNode2D NestedTensor → dense')\nelse:\n    print('PATCH ALREADY APPLIED or pattern not found — leaving file unchanged')\nEOF",
    )
