...
image = (
    ...
    .run_commands(
        ...
        "python3 - <<'EOF'\nimport sys\np = '/ComfyUI/custom_nodes/Comfyui_Minimax_h3_latent_Upscaler/nodes/minimax_h3_latent_upscaler_2d.py'\ntry:\n    with open(p) as f:\n        src = f.read()\nexcept FileNotFoundError:\n    print(f'WARN: {p} not found — skipping upscaler patch', file=sys.stderr)\n    sys.exit(0)\nold = '        s = latent[\"samples\"].clone()\\n        orig_dtype = s.dtype'\nnew = '''        samples_in = latent[\"samples\"]\n        # DEBUG 2026-09-18 (LowUpscaled revival): print real type + attrs\n        # coming from H3 sampler (node 151). Revert after diagnosis.\n        print(f\"[UPSCALER-DEBUG] type={type(samples_in).__module__}.{type(samples_in).__name__}\",\n              f\"shape={getattr(samples_in, 'shape', None)}\",\n              f\"has_is_nested={hasattr(samples_in, 'is_nested')}\",\n              f\"has_to_padded_tensor={hasattr(samples_in, 'to_padded_tensor')}\",\n              f\"has_clone={hasattr(samples_in, 'clone')}\",\n              f\"has_to_dense={hasattr(samples_in, 'to_dense')}\",\n              f\"is_nested_val={getattr(samples_in, 'is_nested', 'N/A')}\",\n              flush=True)\n        # Patch 2026-09-18 (R.109g): filter NT to 5D video only, pad T, stack. R.109c: NT->to_padded_tensor -> 'not NestedTensor'. R.109d: list->to_padded_tensor -> 'not list'. R.109e: stack() failed because NT mixes 5D video [1,24,T,H,W] + 4D audio [1,57,2,150] constituents. R.109f: list-return -> Upscaler does `s = samples_in.clone()` on list -> AttributeError. R.109g: filter constituents to ndim==5 only (Upscaler is video-only), pad T, stack -> single dense tensor.\n        if hasattr(samples_in, 'is_nested') and samples_in.is_nested:\n            import torch as _torch\n            # R.109g: filter to 5D-only constituents (Upscaler is video-only; sampler emits video+audio NT). Pad temporal dim to max_t, stack along dim 0 -> single dense tensor. samples_in.clone() then works.\n            components = [c for c in samples_in.unbind() if c.ndim == 5]\n            if len(components) == 0:\n                components = list(samples_in.unbind())  # fallback if no 5D (should not happen)\n            max_t = max(c.shape[2] for c in components)\n            padded = []\n            for c in components:\n                pad_amt = max_t - c.shape[2]\n                if pad_amt > 0:\n                    # F.pad pad pairs from END. For 5D [B,C,T,H,W]: (W_l,W_r, H_l,H_r, T_l,T_r, C_l,C_r, B_l,B_r)\n                    c = _torch.nn.functional.pad(c, (0, 0, 0, 0, 0, pad_amt, 0, 0, 0, 0), value=0.0)\n                padded.append(c)\n            samples_in = _torch.stack(padded, dim=0)  # R.109g — single dense tensor, [B, C, T, H, W]\n        s = samples_in.clone()\n        orig_dtype = s.dtype'''\nif old in src:\n    src = src.replace(old, new)\n    with open(p, 'w') as f:\n        f.write(src)\n    print('Patched: MinimaxH3LatentUpscalerNode2D NestedTensor → dense')\nelse:\n    print('PATCH ALREADY APPLIED or pattern not found — leaving file unchanged')\nEOF",
    )
